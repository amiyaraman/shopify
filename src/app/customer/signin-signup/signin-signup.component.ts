      ...
      .subscribe({
        ...
        data=data[0];
          if (
            ...
            sessionStorage.setItem('user_session_id', data.id.toString()); // Ensure ID is converted to string
            sessionStorage.setItem('role', data.role);

            const redirectUrl =
              data.role === 'seller' ? '/seller-dashboard' : '/buyer-dashboard';
        ...
        error: (error) => {
          ...
          // Handle login errors gracefully (e.g., display user-friendly error message)
        },
      });
  }
}
